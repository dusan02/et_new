...
async function checkDailyResetState(description) {
  return new Promise((resolve) => {
    ...
    child.on("close", (code) => {
      if (code === 0) {
        console.log(
          `✅ ${description} - Daily reset completed, proceeding with fetch`
        );
        resolve(true);
      } else if (code === 2) {
        console.log(
          `⚠️ ${description} - Auto-repair needed, proceeding with fetch`
        );
        resolve(true);
      } else {
        console.log(
          `⏸️ ${description} - Daily reset not completed, skipping fetch`
        );
        resolve(false);
      }
    });
    ...
    if (earningsCode === 0) {
      ...
      marketChild.on("close", (marketCode) => {
        ...
        if (marketCode === 0) {
          ...
          console.error(
            `❌ ${description} (market) failed, skipping cache clear`
          );
        }
      });
    } else {
